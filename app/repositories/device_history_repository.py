...
class DeviceHistoryRepository:
    ...
    def get_all(self):
        return self.db.query(DeviceHistory).options(joinedload(DeviceHistory.device)).all()

    def get_paginated(self, page: int = 1, page_size: int = 10):
        offset = (page - 1) * page_size
        query = self.db.query(DeviceHistory).options(joinedload(DeviceHistory.device))
        total = query.count()
    ...
    def get_all_by_device(self, device_id: int):
        return self.db.query(DeviceHistory).options(joinedload(DeviceHistory.device)).filter(DeviceHistory.device_id == device_id).all()